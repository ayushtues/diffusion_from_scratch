...
class Diffusion(nn.Module):
    def __init__(self, sqrt_alpha_hat_ts, sqrt_alpha_hat_ts_2, alpha_ts, beta_ts, n_channels, n_classes, bilinear=False):
    ...
    def forward(self, x, t, t_embed, eps):
      c1 = torch.gather(self.sqrt_alpha_hat_ts, 0, t).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) # TODO, move this to the dataset itself instead of using gather
      c2 = torch.gather(self.sqrt_alpha_hat_ts_2, 0, t).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def sample(self, device):
      ...
      for i in reversed(range(1000)):
        ...
        eps_pred = (self.alpha_ts_2[i].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) / self.sqrt_alpha_hat_ts_2[i].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) ) * eps_pred
        print_stats(x, f"eps_pred2_{i}")
        x = x - eps_pred
        x = x * (1/self.sqrt_alpha_ts[i].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
        if i!=0:
